# data/clamps_sounding_fetcher.py
# On-demand fetcher for NSSL CLAMPS DL Truck soundings via THREDDS fileServer.
#
# Catalog URL (XML):
#   https://data.nssl.noaa.gov/thredds/catalog/FRDD/CLAMPS/dltruck/dltruck1/
#   ingested/dltrucksonderawDL1.a1/catalog.xml
#
# File format (.skewT):
#   %TITLE%
...
#   %END%
#
# Up to 12 hours of files are fetched; sorted chronologically so the scrubber
# moves oldest → newest.
...
# Set to a fileServer URL string to test with a specific file, bypassing the
...
def _fetch_sounding_set() -> SoundingSet:
    ...
    # ── Test mode: fetch a single hardcoded file, skipping catalog ────────────
    if _TEST_FILE_URL:
    ...
    # ── Find .skewT datasets within the lookback window ───────────────────────
